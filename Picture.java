...
public class Picture extends SimplePicture 
{
  ...
  public void mirrorDiagonal(int mirrorPoint)
{
    Pixel[][] pixels = this.getPixels2D();
    int rowLimit = Math.min(pixels.length, pixels[0].length);
    ...
    for (int row = 0; row < rowLimit; row++)
    {
        for (int col = 0; col < row; col++)
        {
            ...
            Pixel rightPixel = pixels[col][row];
            rightPixel.setColor(leftPixel.getColor());
        }
    }
}
  ...
  {
        Pixel[][] pixels = this.getPixels2D();
        for(int i = 223; i < 334; i++)
        {
            for(int j = 217;j < 350; j++)
            {
                    colormove( pixels[i][350-j+350], pixels[i][j]);
            }
        }
  }
  ...
  {
        Pixel[][] pixels = this.getPixels2D();
        for(int i = 98; i < 169; j++)
        {
            for(int j = 155; j < 191; i++)
            {
                    colormove(pixels[191-j+191][i], pixels[j][i]);
            }
        }
        
        for(int c = 238; c < 296;c++)
        {
            for(int k = 155; k < 191; k++)
            {
                    colormove(pixels[191-k+191][c], pixels[k][c]);
            }
        }
  }
  
  
public void copyPicture(Picture source, int destRow, int destCol, int sourceRow, int sourceCol, int sourceEndRow, int sourceEndCol)
{
    Pixel srcPixel = null;
    Pixel dstPixel = null;
    ...
    for (int row = sourceRow, destRow = destRow; row < sourceEndRow && row < sourcePixels.length && destRow < destPixels.length; row++, destRow++)
    {
        for (int col = sourceCol, destCol = destCol; col < sourceEndCol && col < sourcePixels[0].length && destCol < destPixels[0].length; col++, destCol++)
        {
            srcPixel = sourcePixels[row][col];
            dstPixel = destPixels[destRow][destCol];
  ...
  for (int row = 0; row < pixels.length; row++)
{
    for (int col = 0; col < pixels[0].length; col++)
    {
        ...
        if (col != pixels[0].length-1) {
            adjacentPixel = pixels[row][col+1];
            adjacentColor = adjacentPixel.getColor();
            if (currentPixel.colorDistance(adjacentColor) > edgeDist)
                currentPixel.setColor(Color.BLACK);
            else
                currentPixel.setColor(Color.WHITE);
        }
        if (row != pixels.length-1) {
            adjacentPixel = pixels[row + 1][col];
            adjacentColor = adjacentPixel.getColor();
            if (currentPixel.colorDistance(adjacentColor) > edgeDist)
                currentPixel.setColor(Color.BLACK);
            else
